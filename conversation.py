...
def _fields_answered_in_message(message: str, ai_result: Dict) -> list:
    """Strictly mark only fields that the latest user message actually answered."""
    ...
    if (
        ...
        or any(term in normalized for term in [
            "תרבות", "היסטוריה", "חופים", "חוף", "רוגע", "טבע", "נופים",
            "הרפתקאות", "חיי לילה", "בילויים", "עירוניות", "שילוב",
    ...
    if (
        extracted.get("travel_party")
        or any(term in normalized for term in [
            "לבד", "זוג", "בן זוג", "בת זוג", "חבר", "חברים", "חברות",
            "משפחה", "ילדים", "לא משנה", "פתוח",
    ...
    if (
        amount
        or extracted.get("budget_amount")
        or any(term in normalized for term in ["תקציב", "שקל", "שח", "₪", "דולר", "יורו", "אירו", "budget", "ils", "usd", "eur", "$", "€"])
    ):
        answered.append("budget")

    if extracted.get("days") or re.search(r"(?:^|[^\d])\d{1,2}\s*(?:ימים?|לילות?|days?|nights?)\b", normalized):
        answered.append("days")

    corrected_by_gemini = str(ai_result.get("corrected_user_text") or "").strip()
    if extracted.get("month") or _parse_month_from_free_text(message) or _parse_month_from_free_text(corrected_by_gemini):
        answered.append("month")

    if (
        extracted.get("pace")
        or any(term in normalized for term in ["רגוע", "מאוזן", "אינטנסיבי", "זמן חופשי", "בלי עומס", "להספיק", "relaxed", "balanced", "intensive"])
    ):
...
def conversational_reply(planner: TravelPlanner, message: str, state: Dict, fetch_live_weather=True):
    ...
    if not pending_before_ai:
        ...
        completed_fields = _sync_completed_fields_from_profile(state.get("wizard_completed") or [], profile)
