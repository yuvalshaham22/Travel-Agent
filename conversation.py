...
def _merge_ai_extraction(profile: Dict, ai_result: Dict, original_message: str) -> None:
    ...
    if days:
        profile["days"] = min(30, max(1, days))
...
def _gemini_suggests_guided_start(ai_result: Dict) -> bool:
    """Use Gemini's own travel/context decision to start a fresh guided flow."""
    if not ai_result or not ai_result.get("ok"):
        return False
    if ai_result.get("is_travel_related") is False:
        return False
    missing = ai_result.get("missing_fields") or []
    if isinstance(missing, str):
        missing = [missing]
    return bool(missing) or ai_result.get("ready_to_recommend") is False
...
def _merge_free_message(planner: TravelPlanner, state: Dict, message: str) -> Dict:
    ...
    """
    corrected = str(message or "").strip()
...
def _sync_completed_fields_from_profile(completed_fields, profile: Dict) -> list:
    """Mark fields already understood earlier in the conversation as completed."""
    synced = []
    for field in completed_fields or []:
        if field in WIZARD_FIELDS and field not in synced:
            synced.append(field)
    for field in WIZARD_FIELDS:
        if field == "experience" and not (profile.get("experience_type") or profile.get("travel_goal")):
            continue
        if _wizard_field_complete(field, profile) and field not in synced:
            synced.append(field)
    return synced


def _fields_answered_in_message(message: str, ai_result: Dict) -> list:
    """Strictly mark only fields that the latest user message explicitly answered.

    Gemini may return helpful guesses, but the guided flow should not skip a
    required question unless the user's latest text contains evidence for it.
    """
    ...
    experience_terms = [
        "\u05ea\u05e8\u05d1\u05d5\u05ea", "\u05d4\u05d9\u05e1\u05d8\u05d5\u05e8\u05d9\u05d4", "\u05d7\u05d5\u05e4\u05d9\u05dd", "\u05d7\u05d5\u05e3",
        "\u05e8\u05d5\u05d2\u05e2", "\u05d8\u05d1\u05e2", "\u05e0\u05d5\u05e4\u05d9\u05dd", "\u05d4\u05e8\u05e4\u05ea\u05e7\u05d0\u05d5\u05ea",
        "\u05d7\u05d9\u05d9 \u05dc\u05d9\u05dc\u05d4", "\u05d1\u05d9\u05dc\u05d5\u05d9\u05d9\u05dd", "\u05e2\u05d9\u05e8\u05d5\u05e0\u05d9\u05d5\u05ea", "\u05e9\u05d9\u05dc\u05d5\u05d1",
        "culture", "beach", "nature", "nightlife", "balanced"
    ]
    if (
        extracted.get("experience_type")
        or extracted.get("travel_goal")
        or any((extracted.get("preferences") or {}).values())
        or any(term in normalized for term in experience_terms)
    ):
        answered.append("experience")

    party_terms = [
        "\u05dc\u05d1\u05d3", "\u05d6\u05d5\u05d2", "\u05d1\u05df \u05d6\u05d5\u05d2", "\u05d1\u05ea \u05d6\u05d5\u05d2",
        "\u05d7\u05d1\u05e8", "\u05d7\u05d1\u05e8\u05d9\u05dd", "\u05d7\u05d1\u05e8\u05d5\u05ea", "\u05de\u05e9\u05e4\u05d7\u05d4",
        "\u05d9\u05dc\u05d3\u05d9\u05dd", "\u05dc\u05d0 \u05de\u05e9\u05e0\u05d4", "\u05e4\u05ea\u05d5\u05d7",
        "solo", "couple", "friends", "family"
    ]
    if any(term in normalized for term in party_terms):
        answered.append("travel_party")

    amount, _ = parse_budget_amount(message, allow_plain=True)
    budget_terms = [
        "\u05ea\u05e7\u05e6\u05d9\u05d1", "\u05e9\u05e7\u05dc", "\u05e9\u05d7", "\u20aa", "\u05d3\u05d5\u05dc\u05e8",
        "\u05d9\u05d5\u05e8\u05d5", "\u05d0\u05d9\u05e8\u05d5", "budget", "ils", "usd", "eur", "$", "\u20ac"
    ]
    if amount or any(term in normalized for term in budget_terms):
        answered.append("budget")

    has_number = bool(re.search(r"(?:^|[^\d])\d{1,2}", normalized))
    duration_terms = ["\u05d9\u05d5\u05dd", "\u05d9\u05de\u05d9\u05dd", "\u05dc\u05d9\u05dc\u05d4", "\u05dc\u05d9\u05dc\u05d5\u05ea", "day", "days", "night", "nights"]
    if has_number and any(term in normalized for term in duration_terms):
        answered.append("days")

    corrected_by_gemini = str(ai_result.get("corrected_user_text") or "").strip()
    if _parse_month_from_free_text(message) or _parse_month_from_free_text(corrected_by_gemini):
        answered.append("month")

    pace_terms = [
        "\u05e8\u05d2\u05d5\u05e2", "\u05de\u05d0\u05d5\u05d6\u05df", "\u05d0\u05d9\u05e0\u05d8\u05e0\u05e1\u05d9\u05d1\u05d9",
        "\u05d6\u05de\u05df \u05d7\u05d5\u05e4\u05e9\u05d9", "\u05d1\u05dc\u05d9 \u05e2\u05d5\u05de\u05e1", "\u05dc\u05d4\u05e1\u05e4\u05d9\u05e7",
        "relaxed", "balanced", "intensive"
    ]
    if any(term in normalized for term in pace_terms):
        answered.append("pace")

    return [field for field in WIZARD_FIELDS if field in answered]
...
def _turn_answers_wizard_field(field: str, message: str, ai_result: Dict) -> bool:
    ...
    if field == "month":
        return bool(_parse_month_answer(message) or _parse_month_answer(corrected_by_gemini) or extracted.get("month"))
    if field == "pace":
...
def _is_unsupported_booking_request(message: str) -> bool:
    """Detect travel-related actions the app cannot actually perform, such as flight search."""
    normalized = str(message or "").lower().strip()
    flight_terms = [
        "טיסה", "טיסות", "כרטיס טיסה", "כרטיסי טיסה", "מצא לי טיסה",
        "תמצא לי טיסה", "לחפש טיסה", "חפש טיסה", "הזמן טיסה", "להזמין טיסה",
        "flight", "flights", "airfare", "plane ticket", "book a flight", "find a flight",
    ]
    hotel_booking_terms = [
        "תזמין לי מלון", "להזמין מלון", "book a hotel", "reserve a hotel",
    ]
    return any(term in normalized for term in flight_terms + hotel_booking_terms)


def _unsupported_booking_response(language: str = "he"):
    if language == "en":
        return {
            "type": "question",
            "language": "en",
            "reply": (
                "I can’t search or book live flights directly. "
                "What I can do is help you choose the most suitable destination and plan it by budget, trip length, month and travel style."
            ),
            "suggestions": ["Help me choose a destination", "Plan Greece by budget", "Build a beach trip"],
            "destinations": [],
            "itinerary": [],
        }
    ...
    return {
        ...
        "language": "he",
        "reply": (
            "אני לא יכולה לחפש או להזמין טיסות בזמן אמת. "
            "כן אוכל לעזור לך לבחור יעד מתאים ולבנות המלצה לפי תקציב, מספר ימים, חודש וסגנון הטיול."
        ),
        "suggestions": ["תעזרי לי לבחור יעד", "תכנני לי טיול ליוון לפי תקציב", "אני רוצה טיול חופים"],
        "destinations": [],
        "itinerary": [],
    }
...
def _exact_month_from_text(text: str):
    """Parse only valid month/season names; typo interpretation belongs to Gemini."""
...
def _parse_month_answer(text: str):
    ...
    return _exact_month_from_text(normalized)


def _parse_month_from_free_text(text: str):
    normalized = str(text or "").lower().strip()
    month = _exact_month_from_text(normalized)
    if month:
        return month
...
def merge_message(planner: TravelPlanner, state: Dict, message: str) -> Dict:
    ...
    if pending == "experience":
        ...
        if number:
            profile["days"] = min(30, max(1, int(number.group())))
    elif pending == "month":
...
def _fallback_pack(destination: dict, profile: dict, itinerary: list) -> dict:
    ...
    def make_item(name, reason, kind):
        category = "restaurants food markets" if kind == "food" else "museums attractions culture"
        clean_name = "" if str(name or "").strip().lower() == str(city or "").strip().lower() else str(name or "").strip()
        search_query = " ".join(part for part in [clean_name, city, country, category] if part)
        return {
            "name": name,
            "reason": reason,
            "kind": kind,
    ...
    if not food_candidates:
        food_candidates = [f"Best restaurants in {city}", f"Food markets in {city}", f"Local cafes in {city}"]
...
def _build_packing_list(destination: dict, profile: dict) -> dict:
    ...
    if preferences.get("beaches", 0) >= 3:
        add(
            "בגד ים ובגד להחלפה" if language == "he" else "Swimwear and a change of clothes",
            "כפכפים ותיק חוף" if language == "he" else "Sandals and a beach bag",
            "מגבת חוף קלה ושקית לבגדים רטובים" if language == "he" else "Light beach towel and a wet-clothes bag",
        )
    if preferences.get("nature", 0) >= 4 or preferences.get("adventure", 0) >= 4:
        add(
            "נעלי שטח וגרביים מתאימות" if language == "he" else "Trail shoes and suitable socks",
            "תיק יום קטן" if language == "he" else "Small daypack",
            "פלסטרים נגד שפשופים וחטיפי אנרגיה" if language == "he" else "Blister plasters and energy snacks",
        )
    if preferences.get("culture", 0) >= 4 or preferences.get("urban", 0) >= 4:
        add(
            "תיק יום קל ולבוש מכבד לאתרי תרבות" if language == "he" else "Light day bag and respectful clothing for cultural sites",
            "נעליים נוחות להליכה בעיר" if language == "he" else "Comfortable shoes for city walking",
        )
    if preferences.get("nightlife", 0) >= 4:
        add(
            "לבוש ערב נוח שמתאים לבילויים" if language == "he" else "Comfortable evening outfit for going out",
            "תיק קטן מאובטח לערב" if language == "he" else "Small secure evening bag",
        )
    if preferences.get("cuisine", 0) >= 4:
        add(
            "רשימת מסעדות שמורות מראש באפליקציה" if language == "he" else "Saved restaurant list on your phone",
            "מקום קטן בתיק לקניות משווקים מקומיים" if language == "he" else "A little bag space for local market finds",
        )
    if preferences.get("wellness", 0) >= 4 or preferences.get("seclusion", 0) >= 4:
        add(
            "בגד נוח לזמן מנוחה וספא" if language == "he" else "Comfortable outfit for downtime or spa",
            "ספר/אוזניות לזמן רגוע" if language == "he" else "Book or headphones for relaxed time",
        )

    pace = profile.get("pace")
    if pace == "relaxed":
        add(
            "תיק קל במיוחד לימים רגועים בלי עומס" if language == "he" else "Extra-light day bag for relaxed days",
            "בגד נוסף נוח לזמן חופשי" if language == "he" else "Extra comfortable outfit for free time",
        )
    elif pace == "intensive":
        add(
            "נעליים שכבר נוחות לך ולא חדשות" if language == "he" else "Broken-in shoes, not new ones",
            "מטען נייד חזק לימים ארוכים" if language == "he" else "Strong power bank for long days",
            "בקבוק מים ותיק גב קטן לכל היום" if language == "he" else "Water bottle and small all-day backpack",
        )
    elif pace == "balanced":
        add(
            "תיק יום בינוני שמתאים גם לעיר וגם לטיולים קצרים" if language == "he" else "Medium day bag for both city time and short outings",
        )
    ...
    if len(items) > 12:
        basics = items[:5]
        weather_items = items[5:9]
        trip_specific = items[9:-2]
        closing_context = items[-2:]
        short_items = basics[:4] + weather_items[:3] + trip_specific[:3] + closing_context
        items = []
        add(*short_items)
    ...
    experience_labels = {
        "beach_relax": "חופים ורוגע",
        "nature_adventure": "טבע והרפתקאות",
        "nightlife_urban": "עירוניות וחיי לילה",
        "romantic_relax": "טיול רומנטי ורגוע",
        "balanced_mix": "שילוב מאוזן",
        "beach_adventure": "חופים ופעילות",
    }
    experience_labels_en = {
        "beach_relax": "beaches and relaxation",
        "nature_adventure": "nature and adventure",
        "nightlife_urban": "city and nightlife",
        "romantic_relax": "romantic relaxed trip",
        "balanced_mix": "balanced mix",
        "beach_adventure": "beaches and activity",
    }
    pace_labels = {"relaxed": "קצב רגוע", "balanced": "קצב מאוזן", "intensive": "קצב אינטנסיבי"}
    pace_labels_en = {"relaxed": "relaxed pace", "balanced": "balanced pace", "intensive": "intensive pace"}
    trip_style = (experience_labels_en if language == "en" else experience_labels).get(profile.get("experience_type"))
    pace_label = (pace_labels_en if language == "en" else pace_labels).get(profile.get("pace"))
    duration_context = f"{days} days" if language == "en" else f"{days} ימים"
    context = " · ".join(value for value in [weather, duration_context, party_label, trip_style, pace_label] if value)
    return {
...
def conversational_reply(planner: TravelPlanner, message: str, state: Dict, fetch_live_weather=True):
    ...
    if not greeting and _is_unsupported_booking_request(message):
        response = _unsupported_booking_response(
            (gemini_result.get("language") if gemini_result.get("ok") else None) or current_language
        )
        _append_history(state, "user", message)
        _append_history(state, "assistant", response["reply"])
        return response, state

    if not greeting and gemini_result.get("ok") and gemini_result.get("is_travel_related") is False:
        response = _out_of_scope_response(gemini_result.get("language") or current_language)
        _append_history(state, "user", message)
        _append_history(state, "assistant", response["reply"])
        return response, state

    if not greeting and not gemini_result.get("ok") and (_is_out_of_scope(message) or _is_casual_non_travel(message)):
        response = _out_of_scope_response(current_language)
        _append_history(state, "user", message)
        _append_history(state, "assistant", response["reply"])
        return response, state

    if state.get("completed") and not pending_before_ai:
        current_language = (gemini_result.get("language") if gemini_result.get("ok") else None) or current_language
        state = {"profile": empty_profile(current_language), "history": [], "mode": "guided", "wizard_completed": []}
        pending_before_ai = None
        profile_before_ai = state["profile"]
        if gemini_result.get("ok"):
            _merge_ai_extraction(profile_before_ai, gemini_result, message)
    ...
    if not pending_before_ai:
        completed_fields = _fields_answered_in_message(corrected_message, gemini_result)
    else:
        completed_fields = list(state.get("wizard_completed") or [])
        for field in _fields_answered_in_message(corrected_message, gemini_result):
            if field not in completed_fields:
                completed_fields.append(field)
    state["wizard_completed"] = completed_fields

    if not state.get("wizard_completed") and not pending_before_ai:
        state["pending_field"] = WIZARD_FIELDS[0]
        reply, suggestions = _guided_question(WIZARD_FIELDS[0], profile.get("language", current_language))
        _append_history(state, "assistant", reply)
        return {
            "type": "question",
            "language": profile.get("language", current_language),
            "reply": reply,
            "suggestions": suggestions,
            "profile": profile,
            "missing_fields": [field for field in WIZARD_FIELDS if field not in completed_fields],
