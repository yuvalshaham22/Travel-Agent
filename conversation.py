...
def conversational_reply(planner: TravelPlanner, message: str, state: Dict, fetch_live_weather=True):
    ...
    if state.get("completed") and not pending_before_ai and _gemini_suggests_guided_start(gemini_result):
        current_language = gemini_result.get("language") or current_language
