...
REQUIRED_FREE_FIELDS = ["experience", "travel_party", "days", "month", "budget", "budget_currency", "pace"]
...
def _guided_question(field: str, language: str = "he") -> tuple[str, list]:
    ...
    if language == "en":
        ...
        questions = {
            "experience": ("איזה סוג חוויה הכי מתאים לך בטיול?", ["תרבות ואוכל", "חופים ורוגע", "טבע והרפתקאות", "עיר וחיי לילה", "שילוב מאוזן"]),
            "travel_party": ("עם מי נוסעים?", ["לבד", "זוג", "חברים", "משפחה"]),
            "budget": ("מה התקציב הכולל המשוער? אפשר גם לכתוב כל סכום חופשי.", ["3,000 ₪", "5,000 ₪", "10,000 ₪"]),
            "budget_currency": ("באיזה מטבע התקציב?", ["שקלים (ILS)", "דולרים (USD)", "אירו (EUR)"]),
            "days": ("לכמה ימים מתכננים את הטיול?", ["3 ימים", "5 ימים", "7 ימים", "10 ימים"]),
            "month": ("באיזה חודש תרצו לנסוע? אפשר גם להקליד חודש אחר.", ["ינואר", "אפריל", "יולי", "אוקטובר"]),
            "pace": ("איזה קצב טיול מתאים לכם?", ["רגוע", "מאוזן", "אינטנסיבי"]),
        }
...
def conversational_reply(planner: TravelPlanner, message: str, state: Dict, fetch_live_weather=True):
    ...
    # Gemini extracts meaning, while the local state machine guarantees that the
    # user sees every required question and that recommendation readiness is consistent.
    if gemini_result.get("ok"):
    ...
    ready_to_recommend = not missing
