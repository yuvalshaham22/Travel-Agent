...
def _candidate_models():
    """Return the requested model first, then supported fallbacks.
    ...
    models = [requested] + GEMINI_FALLBACK_MODELS
    unique = []
    for model in models:
...
def _is_retryable_model_error(status_code: int, error_text: str) -> bool:
...
def _call_gemini_json(prompt: str, temperature: float = 0.2, max_tokens: int = 1800) -> dict:
    ...
    if not api_key:
        return {"ok": False, "error": "missing_api_key", "had_api_key": False}
    ...
    for model in _candidate_models():
        ...
        try:
            ...
            if _is_retryable_model_error(exc.code, error_text):
                continue
            return {"ok": False, "error": last_error, "had_api_key": True, "tried_models": tried}
...
Typo and context examples:
- "חופימ", "חופעם", "חופיים" -> beaches / חופים.
- "תרבוט" -> תרבות.
- "באלי" in Hebrew travel context often means "בא לי", unless the context clearly means Bali island.
- Month typos: "רפריל", "אפרל", "אפרייל" -> אפריל / month 4; "ינוארר" -> ינואר; "ספטממבר" -> ספטמבר.
- If the previous assistant asked about month and the user writes "רפריל", extract month=4 and continue.
- If the previous assistant asked about budget and the user writes "12", this is ambiguous; ask whether they mean 12,000 ₪/$/€ or something else.
- If the previous assistant asked about days and the user writes "12", extract days=12.
