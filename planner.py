...
def parse_budget_amount(text: str, language: Optional[str] = None, allow_plain: bool = False):
    ...
    for phrase, amount in {**HEBREW_THOUSANDS, **ENGLISH_THOUSANDS}.items():
        if phrase in normalized:
            return amount, currency or ("ILS" if language == "he" else "USD")
    ...
    if amount < 100:
        return None, None
    return int(round(amount)), currency or ("ILS" if language == "he" else "USD")
...
class TravelPlanner:
    ...
    def parse_request(self, text: str) -> Dict:
        ...
        days = min(14, max(1, int(days_match.group(1)))) if days_match else None
